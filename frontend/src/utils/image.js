import Compressor from 'compressorjs'

export const DEFAULT_IMAGE_URL = 'data:image/svg+xml;base64,'

export const DEFAULT_IMAGE_WIDTH = 128
export const DEFAULT_IMAGE_HEIGHT = 128
export const DEFAULT_IMAGE_SIZE = 1024

export const MAX_IMAGE_WIDTH = 1920
export const MAX_IMAGE_HEIGHT = 1920
export const MAX_IMAGE_SIZE = 5 * 1024 * 1024

const TYPE_DEFAULT_IMAGES = {
  string: 'data:image/svg+xml;base64,',
  bow: 'data:image/svg+xml;base64,',
  pick: 'data:image/svg+xml;base64,',
  rosin: 'data:image/svg+xml;base64,',
  capo: 'data:image/svg+xml;base64,',
  strap: 'data:image/svg+xml;base64,',
  cleaner: 'data:image/svg+xml;base64,',
  other: DEFAULT_IMAGE_URL
}

export function compressImage(file, options = {}) {
  return new Promise((resolve, reject) => {
    new Compressor(file, {
      quality: options.quality || 0.7,
      maxWidth: options.maxWidth || MAX_IMAGE_WIDTH,
      maxHeight: options.maxHeight || MAX_IMAGE_HEIGHT,
      mimeType: 'image/jpeg',
      convertSize: 1024 * 1024,
      success: (compressedFile) => {
        resolve(new File([compressedFile], file.name.replace(/\.[^.]+$/, '.jpg'), { type: 'image/jpeg' }))
      },
      error: (err) => {
        reject(err)
      }
    })
  })
}

export async function compressImageList(files, options) {
  const results = []
  for (const file of files) {
    try {
      const compressed = await compressImage(file, options)
      results.push(compressed)
    } catch {
      results.push(file)
    }
  }
  return results
}

export function formatFileSize(bytes) {
  if (bytes < 1024) return bytes + ' B'
  if (bytes < 1024 * 1024) return (bytes / 1024).toFixed(1) + ' KB'
  return (bytes / (1024 * 1024)).toFixed(2) + ' MB'
}

export function isValidImageUrl(url) {
  if (!url) return false
  return url.startsWith('http://') || url.startsWith('https://') || url.startsWith('data:image/')
}

export function getDefaultImageUrl(typeCode) {
  if (!typeCode) return DEFAULT_IMAGE_URL
  return TYPE_DEFAULT_IMAGES[typeCode] || DEFAULT_IMAGE_URL
}

export function resolveImageUrl(imageUrl, typeCode) {
  if (isValidImageUrl(imageUrl)) {
    return imageUrl
  }
  return getDefaultImageUrl(typeCode)
}

export function resolveImageWidth(width) {
  if (!width || width <= 0) return DEFAULT_IMAGE_WIDTH
  return Math.min(width, MAX_IMAGE_WIDTH)
}

export function resolveImageHeight(height) {
  if (!height || height <= 0) return DEFAULT_IMAGE_HEIGHT
  return Math.min(height, MAX_IMAGE_HEIGHT)
}

export function resolveImageSize(size) {
  if (!size || size <= 0) return DEFAULT_IMAGE_SIZE
  return Math.min(size, MAX_IMAGE_SIZE)
}

export function validateImageDimension(width, height) {
  if (width == null || height == null) return true
  return width > 0 && width <= MAX_IMAGE_WIDTH && height > 0 && height <= MAX_IMAGE_HEIGHT
}

export function validateImageSize(size) {
  if (size == null) return true
  return size > 0 && size <= MAX_IMAGE_SIZE
}

export function getImageMeta(accessory) {
  if (!accessory) {
    return {
      imageUrl: DEFAULT_IMAGE_URL,
      imageWidth: DEFAULT_IMAGE_WIDTH,
      imageHeight: DEFAULT_IMAGE_HEIGHT,
      imageSize: DEFAULT_IMAGE_SIZE
    }
  }
  return {
    imageUrl: resolveImageUrl(accessory.imageUrl, accessory.typeCode),
    imageWidth: resolveImageWidth(accessory.imageWidth),
    imageHeight: resolveImageHeight(accessory.imageHeight),
    imageSize: resolveImageSize(accessory.imageSize)
  }
}

export async function getImageDimensions(file) {
  return new Promise((resolve, reject) => {
    const img = new Image()
    img.onload = () => {
      resolve({ width: img.width, height: img.height })
    }
    img.onerror = () => {
      reject(new Error('Failed to load image'))
    }
    img.src = URL.createObjectURL(file)
  })
}
